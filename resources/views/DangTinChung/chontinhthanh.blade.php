...
<form action="{{url('chon-quan-huyen')}}"  method="POST"  role="form" name="form4">
  {{ csrf_field()}}
   <div class="luan-panel-group">
	    <div class="luan-panel luan-panel-default">
	        <div class="form-group" style="margin:0px; width: 100%;">
	        	<div class="label-1">
		        	<label id="label-01" for="sel1">Tỉnh thành</label>
		        </div>
		        <div class="danh-muc-dang-tin">
		        	<input id="hidden-input-common" type="hidden" name="tinhthanh" value="">
		        	<?php
		            	foreach($tinhthanh as  $value){
		            		$item = "<button class='item-danhmuc' onclick=\"get_value_final('".$value->tenTinhThanh."')\" type='button'><span class='vertical-align-middle'>";
		          			$item .= $value->tenTinhThanh."</span>";
